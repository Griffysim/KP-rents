...
async function getUtilityRates() {
  // Robust single-row handling: ensure the specific row exists, then return typed rates
  async function ensureUtilityRatesRow() {
    // Try to fetch the known config row by id first
    const { data: byIdData, error: byIdError } = await supabaseClient
      .from('utility_rates')
      .select('*')
      .eq('id', UTILITY_RATES_ROW_ID)
      .limit(1);

    if (byIdError) throw byIdError;
    if (byIdData && byIdData.length > 0) return byIdData[0];

    // If the specific id row isn't present, fall back to the first row if any
    const { data, error } = await supabaseClient
      .from('utility_rates')
      .select('*')
    ...
    // No row yet → create one with defaults and the provided id
    const defaults = { id: UTILITY_RATES_ROW_ID, electricity_rate: 0.15, water_rate: 8.5 };
  ...
  try {
    const row = await ensureUtilityRatesRow();
    return {
      electricity_rate: Number(row.electricity_rate ?? 0.15),
      water_rate: Number(row.water_rate ?? 8.5),
    };
  } catch (error) {
    console.error('Error fetching utility rates:', error);
    return { electricity_rate: 0.15, water_rate: 8.5 };
  }
}

async function updateUtilityRates(rates) {
  // Update the single config row (partial updates allowed)
  try {
    // Ensure the named row exists and then update it directly by id
...
// Partial update helper used by the per-rate buttons
async function updateUtilityRatesPartial(partialRates) {
  const row = await (async function ensureUtilityRatesRow() {
    const { data, error } = await supabaseClient
      .from('utility_rates')
      .select('*')
      .eq('id', UTILITY_RATES_ROW_ID)
...
function readRateOrThrow(inputId, label) {
  const el = document.getElementById(inputId);
  const value = parseFloat(el?.value);

  if (!Number.isFinite(value) || value < 0) {
    throw new Error(`${label} rate must be a valid number (0 or higher).`);
  }
  return value;
}

// Separate update actions (triggered by the new buttons)
async function updateElectricityRate() {
  try {
    const electricity_rate = readRateOrThrow('electricityRate', 'Electricity');
    await updateUtilityRatesPartial({ electricity_rate });
    showAlert('✅ Electricity rate updated successfully');
    await loadSettings(); // refresh values from DB
  } catch (error) {
    showAlert('❌ ' + error.message, 'danger');
  }
}

async function updateWaterRate() {
  try {
    const water_rate = readRateOrThrow('waterRate', 'Water');
    await updateUtilityRatesPartial({ water_rate });
    showAlert('✅ Water rate updated successfully');
    await loadSettings(); // refresh values from DB
  } catch (error) {
    showAlert('❌ ' + error.message, 'danger');
  }
...
async function generateInvoiceWithAI(payload) {
  ...
  if (!res.ok) {
    ...
    throw new Error('Invoice generation failed: ' + err);
  }
...
// Helper to test email sending from the app (call from console)
async function testSendInvoiceEmail() {
  const payload = {
    to: 'test@example.com',
    subject: 'Test Invoice Email',
    body: 'This is a test invoice email from KP-Rents.',
...
async function generateAndSendInvoice() {
  ...
  try {
    ...
    let invoiceResult;
    try {
      invoiceResult = await generateInvoiceWithAI(invoicePayload);
      console.log('✅ Invoice generation result:', invoiceResult);
    } catch (genErr) {
      console.error('❌ Invoice generation failed:', genErr);
      throw new Error('Invoice generation failed: ' + (genErr.message || genErr));
    }

    if (!invoiceResult || !invoiceResult.invoiceText) {
      console.error('❌ Empty invoice text returned:', invoiceResult);
      throw new Error('Invoice generation returned no content');
    }
    ...
    try {
      ...
      console.log('✅ Invoice saved to DB:', invoiceRecord.id);
    } catch (saveErr) {
      console.error('❌ Saving invoice failed:', saveErr);
      throw new Error('Failed to save invoice: ' + (saveErr.message || saveErr));
    }
    ...
    try {
      console.log('📧 Sending email with payload:', { to: emailPayload.to, subject: emailPayload.subject });
      await sendInvoiceEmail(emailPayload);
      console.log('✅ Email send completed');
    } catch (emailErr) {
      console.error('❌ Sending email failed:', emailErr);
      throw new Error('Failed to send invoice email: ' + (emailErr.message || emailErr));
    }
...
async function saveSettings() {
  ...
  try {
    ...
    showAlert('✅ Landlord details saved successfully');
  } catch (error) {
    showAlert('❌ Error saving landlord details: ' + error.message, 'danger');
  }
}
